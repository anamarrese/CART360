...
void live()
{
  //intialize int variable that will store the combined value of testNote + offSetFrequency
  int combinedNoteValue;

   offsetFrequency = getPhotoFrequency();
   // output sound

   //read note input from user and store into testNote
    testNote = analogRead(NOTE_IN_PIN);
      //Serial.println(testNote);
      //if statement so buzzer does not always make sound (min sound)
  if (testNote > 8){
    //store the sum into combinedNoteValue
    combinedNoteValue = offsetFrequency + testNote;

    //output sound using tone()function and frequency = combinedNoteValue
    tone(BUZZER_PIN, combinedNoteValue, 100);
  }
}
...
void recordWithDuration()
{
  ...
  if (countNotes < MAX_NOTES)
  {
    ...
    else if (testNote > theAdjuster && (timePassed > MAX_PRESS_TIME))
    {
      ...
      //noTone() stops buzzer sound when calling it
      noTone(BUZZER_PIN);
    }
     /*** STATE C::: *******************************************
...
void startUpTimer()
{
  ...
  //reset activeNoteButton by changing it to true so it enters correct if statement in recordWithDuration()
...
void playCurrentNote()
{
  ...
  int combinedNoteValue;
   ...
   //store sum into combinedNoteValue
   combinedNoteValue = offsetFrequency + testNote;
...
void updateArraysWithNoteAndTimings()
{
 ...
 //if statement to make sure we have not attained maximum number of elements in notes[] array
 if (countNotes < MAX_NOTES){

    //store sum of running average value and tesNote into integer variable called combinedValue
      int combinedValue = averageOffsetFreq + testNote;

      //assign combinedValue to element in notes[] array
      notes[countNotes] = combinedValue;

      //assign timePassed value to element in durations[] array
      durations[countNotes] = timePassed;

      //increment countNotes to move on to next element in array
      countNotes++;
 }
...
int getPhotoFrequency()
{
  ...
  //read photocell frequency and store it in int variable 
  int photocellFrequency = analogRead(PHOTO_PIN);
  
 // Serial.println(photocellFrequency);

 //divide value by 4 to scale it down between 0-255 since number is up to 1023
 int scaledValue = photocellFrequency/4;
...
int getRunningAverage()
{
 ...
 //store offsetFrequency integer value in element of runningAverageBuffer[] array
 runningAverageBuffer[nextCount] = offsetFrequency;

 //increment nextCount
 nextCount++;

//reset nextCount if we have reached more than 16 inputs
  ...
  //for each photocell value (element) in the array, add it onto sum variable 
  for(int i=0; i< RUNNING_SAMPLES; i++){ 
    sum = sum + runningAverageBuffer[i];  
    } 

   //get average by dividing sum by number of non-empty elements in array
   averageOffsetFreq = sum / RUNNING_SAMPLES; 

   //small delay
   delay(100); 

   //return running average of photocell value
...
int brightness;
  ...
  //if statements to change brightness with frequenchy change.
  //I "randomized" the brightness depending on the int col
  //because it makes it clearer to see the brightness change this way.
  if (col >= 0 && col < 30){
    brightness  = 5;
  }
  ...
  //divide value of note and photocell by 4 to make sure it is within 0-255
  
  analogWrite(LED_PIN_R, 0);   // Turn on the LED -R
  analogWrite(LED_PIN_G, brightness);   // Turn on the LED -G
  analogWrite(LED_PIN_B, 0);   // Turn on the LED -B
...
//continue while loop while boolean is true
  while (playTune){

    //for each note played, output buzzer sound for specific timePassed and change brightness of LED 
    for(int i=0; i<countNotes;i++){
    ...
    //if user presses on mode button, stop the boolean to go to next mode and break out of for loop
     if(digitalRead(BUTTON_MODE_PIN) ==HIGH){
     playTune = false;
     break;
     }
